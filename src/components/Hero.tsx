...
function genParticles(options: Options, width: number, height: number) {
  return populateFromPath(
    ...
    (ctx, scale) => {
      ...
      if (options.text) {
        ctx.font = `${120 * scale}px ChocoladineDemo`;
        ctx.fillStyle = 'cyan';
...
function TextCanvas({
  ...
  const [gl, canvas, setCanvas] = useCanvas({
    ...
    resize(gl: WebGL2RenderingContext) {
      ...
      // 🔄 Regenerar partículas con el nuevo tamaño del canvas
      initializeParticles(gl, gl.canvas as HTMLCanvasElement);
    }
  });
  ...
  return <canvas ref={setCanvas} className='am-canvas' />;
}
...
export default function TextCanvasWrapper() {
  ...
  useEffect(() => {
...

    {
      ...
      optionsFolder.addButton({ title: 'Reset to Defaults' }).on('click', () => {
  ...
  return (
    <section className='am-hero'>
      ...
      <img src="./assets/reactLogo.svg" alt="logo react" className='am-logo-react'></img>
    </section>
  );
}
